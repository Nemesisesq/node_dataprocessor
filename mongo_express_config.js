...
if (typeof process.env.MONGODB_URI == 'string') {
    var parsed = url.parse(process.env.MONGODB_URI)
    mongo.db = parsed.path.replace("/", "")
    mongo.host = parsed.hostname
    if (parsed.auth) {
        mongo.password = parsed.auth.split(":")[1]
        mongo.username = parsed.auth.split(":")[0]

    }
    mongo.port = parsed.port
    mongo.ssl = false
    mongo.url = parsed.href
}
